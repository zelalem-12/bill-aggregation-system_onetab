...
const validToken = "a8c2f3d9b5e7a6d3f0b9c1a8e2d7b5c9" //Token given by the water provider to the user
...
func getBills(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(waterBills)
}

func main() {
	http.HandleFunc("/api/v1/bills", authMiddleware(getBills))

	log.Println("Water mock server running on http://localhost:5002")
